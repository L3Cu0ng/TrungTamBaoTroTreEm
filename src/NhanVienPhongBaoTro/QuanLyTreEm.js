...
const VNBTChildManagement = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Check for Unix epoch date and set it to "Không có ngày sinh"
          ...
          // Format trạng thái
          const formattedTrangThai = child.trangthai === '1' ? 'Chưa được nhận nuôi'
            : child.trangthai === '2' ? 'Đang chờ nhận nuôi'
            : child.trangthai === '3' ? 'Đã được nhận nuôi'
            : 'Unknown';
  ...
  const handleAdd = () => {
    navigate('/vnpbt-children-management/addChild');
  };
  ...
  const handleEdit = (id) => {
    navigate(`/nvpbt-children-management/child-detail/${id}`);
  };
...
export default VNBTChildManagement;
